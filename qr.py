...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if time.time() < last_detected + 10:
            time.sleep(1)
            # print('Sleep for 10s. Category: {}'.format(category))
            continue
                ...
                for c in det[:, -1].unique():
                    ...
                    if names[int(c)] == 'person':
                        ...
                        if working == False: 
                            last_presence_detected = time.time()
                            working = True
                            print('Presence detected. Category: {}'.format(category))
                        time.sleep(1.0)
                        
                
                if objs_detected == False and working==True:
                    print('Subject gone? Category: {}'.format(category))
                    attempts += 1

                if time.time() - last_presence_detected > 600 and working == True:
